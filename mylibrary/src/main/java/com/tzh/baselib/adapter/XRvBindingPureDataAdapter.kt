...
abstract class XRvBindingPureDataAdapter<T>(@LayoutRes val itemLayout: Int = 0) : XRvPureBindingAdapter() {
    ...
    open fun addData(data: T, isNotify: Boolean = true) {
        addData(listData.size, data, isNotify)
    }
    ...
    open fun addData(position: Int, data: T, isNotify: Boolean = true) {
        ...
        if (isNotify) {
            ...
            notifyDataPositionsChanged(position)
        }
    }
    ...
    open fun addDatas(datas: MutableList<T>, isNotify: Boolean = true) {
        addDatas(listData.size, datas, isNotify)
    }
    ...
    open fun removeData(index: Int): Boolean {
        return if (index >= 0) {
            if (index < listData.size) {
                ...
                notifyDataPositionsChanged(index)
                true
            } else {
                false
            }
    ...
    private fun notifyDataPositionsChanged(position: Int) {
        // Rebind shifted rows and the predecessor whose divider may have changed.
        val start = (position - 1).coerceAtLeast(0)
        val count = listData.size - start
        if (count > 0) notifyItemRangeChanged(start, count)
    }
