...
class BaseNetWork : ObservableTransformer<Any?, Any?> {
    ...
    private fun initRetrofit() {
        ...
        client.addNetworkInterceptor(metadataLoggingInterceptor())
    ...
    companion object {
        // Never log URLs, authorization headers or request/response bodies.
        private fun metadataLoggingInterceptor() = Interceptor { chain ->
            val request = chain.request()
            val response = chain.proceed(request)
            LogUtils.d("Http", request.method + " " + response.code)
            response
        }
